...
// Вариант A
usePivotLongExitA        = input.bool(true,  "Вариант A", group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitAFirst      = input.string("HH", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitASecond     = input.string("LH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
// Вариант B
usePivotLongExitB        = input.bool(false, "Вариант B", group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitBFirst      = input.string("HL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitBSecond     = input.string("LL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
// Вариант C
usePivotLongExitC        = input.bool(false, "Вариант C", group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitCFirst      = input.string("LL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitCSecond     = input.string("HL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
// Вариант D
usePivotLongExitD        = input.bool(false, "Вариант D", group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitDFirst      = input.string("HL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
pivotLongExitDSecond     = input.string("HH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Лонга")
// Общее окно
...
// Вариант A
usePivotLongEntryA       = input.bool(false, "Вариант A", group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryAFirst     = input.string("LL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryASecond    = input.string("HL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
// Вариант B
usePivotLongEntryB       = input.bool(false, "Вариант B", group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryBFirst     = input.string("HL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryBSecond    = input.string("HH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
// Вариант C
usePivotLongEntryC       = input.bool(false, "Вариант C", group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryCFirst     = input.string("LL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryCSecond    = input.string("LL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
// Вариант D
usePivotLongEntryD       = input.bool(false, "Вариант D", group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryDFirst     = input.string("HH", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
pivotLongEntryDSecond    = input.string("LH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Лонга")
// Общее окно
...
// Вариант A
usePivotShortExitA       = input.bool(true,  "Вариант A", group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitAFirst     = input.string("LL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitASecond    = input.string("HL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
// Вариант B
usePivotShortExitB       = input.bool(false, "Вариант B", group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitBFirst     = input.string("HL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitBSecond    = input.string("HH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
// Вариант C
usePivotShortExitC       = input.bool(false, "Вариант C", group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitCFirst     = input.string("HH", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitCSecond    = input.string("HH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
// Вариант D
usePivotShortExitD       = input.bool(false, "Вариант D", group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitDFirst     = input.string("LH", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
pivotShortExitDSecond    = input.string("LL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Закрытие Шорта")
// Общее окно
...
// Вариант A
usePivotShortEntryA      = input.bool(false, "Вариант A", group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryAFirst    = input.string("HH", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryASecond   = input.string("LH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
// Вариант B
usePivotShortEntryB      = input.bool(false, "Вариант B", group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryBFirst    = input.string("LH", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryBSecond   = input.string("LL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
// Вариант C
usePivotShortEntryC      = input.bool(false, "Вариант C", group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryCFirst    = input.string("LL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryCSecond   = input.string("HL", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
// Вариант D
usePivotShortEntryD      = input.bool(false, "Вариант D", group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryDFirst    = input.string("HL", "Первый", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
pivotShortEntryDSecond   = input.string("HH", "Второй", options=["HH","LH","HL","LL"], group="HH/LH Фильтр: Открытие Шорта")
// Общее окно
...
// Вспомогательная функция: поиск одной пары пивотов
...
pivotMultiPatternDetector(useA, firstA, secondA, useB, firstB, secondB, useC, firstC, secondC, useD, firstD, secondD, windowBack, windowFwd, signalBar) =>
    // Проверяем сколько вариантов включено
    enabledVariantCount = 0
    if useA
        enabledVariantCount := enabledVariantCount + 1
    if useB
        enabledVariantCount := enabledVariantCount + 1
    if useC
        enabledVariantCount := enabledVariantCount + 1
...
if enableLong and isFirstGreenAfterRed and not longIsOpen
...
if lBuySignal and not lBuyBlocked and strategy.equity > 0
    // Проверка пивот-фильтра для входа в лонг
    pivotEntryOk = true
    ...
    if anyVariantEnabled
        ...
        [pivotFound, pivotExpl, _lEntryMaxPivot] = pivotMultiPatternDetector(
             ...
             pivotLongEntryWinBack, pivotLongEntryWinFwd, searchFrom)
        if pivotFound
    ...
    if pivotEntryOk
        ...
        if allowFlip and shortIsOpen
            ...
            // Reset all short state variables
            shortIsOpen := false
            sStopActive := false
            sTrailStop  := na
            sPhase      := 0
...
if longIsOpen and lPhase == 0 and not lStopActive and (na(lEntryBar) or bar_index > lEntryBar)
    ...
    if lSellMet
        ...
        anyVariantEnabled = usePivotLongExitA or usePivotLongExitB or usePivotLongExitC or usePivotLongExitD
        if anyVariantEnabled
            lPhase    := 2
            lHhFound  := false
...
if longIsOpen and lPhase == 2
    ...
    [pivotFound, pivotExpl, lMaxPivotBar] = pivotMultiPatternDetector(
         ...
         pivotLongExitWinBack, pivotLongExitWinFwd, searchStart)
...
if lStopActive and longIsOpen
    ...
    if close < lTrailStop
        ...
        // Reset all long state variables
        longIsOpen  := false
        lStopActive := false
        lTrailStop  := na
        lPhase      := 0
...
if enableShort and isFirstRedAfterGreen and not shortIsOpen
...
if sEntrySignal and not sEntryBlocked and strategy.equity > 0
    // Проверка пивот-фильтра для входа в шорт
    pivotEntryOk = true
    ...
    if anyVariantEnabled
        ...
        [pivotFound, pivotExpl, _sEntryMaxPivot] = pivotMultiPatternDetector(
             ...
             pivotShortEntryWinBack, pivotShortEntryWinFwd, searchFrom)
        if pivotFound
...
if shortIsOpen and sPhase == 0 and not sStopActive and (na(sEntryBar) or bar_index > sEntryBar)
    ...
    if sCloseMet
        ...
        anyVariantEnabled = usePivotShortExitA or usePivotShortExitB or usePivotShortExitC or usePivotShortExitD
        if anyVariantEnabled
            sPhase      := 2
            sLlFound    := false
...
if shortIsOpen and sPhase == 2
    ...
    [pivotFound, pivotExpl, sMaxPivotBar] = pivotMultiPatternDetector(
         ...
         pivotShortExitWinBack, pivotShortExitWinFwd, searchStart)
